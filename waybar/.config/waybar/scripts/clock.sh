...
# Colors
TODAY_BG="#26A65B"     # green, matches waybar battery charging color
TODAY_FG="#000000"
WEEKDAY_FG="#888888"
DAY_FG="#ffffff"
HEADER_FG="#ffffff"
...
# Header (month year) in Inter, bold
header=$(printf '%s' "$cal_output" | sed -n '1p')
header_span=$(printf '<span font_family="Inter" size="16000" weight="bold" foreground="%s">%s</span>' "$HEADER_FG" "$header")

# Weekday row in JetBrains Mono, gray
weekdays=$(printf '%s' "$cal_output" | sed -n '2p')
weekday_span=$(printf '<span font_family="JetBrains Mono" size="11000" foreground="%s">%s</span>' "$WEEKDAY_FG" "$weekdays")
...
while IFS= read -r line; do
    ...
    while [ $i -lt 21 ]; do
        ...
        if [ "$num" = "$today_unpad" ]; then
            ...
            row+=$(printf '<span font_family="JetBrains Mono" size="11000" foreground="%s">%s</span>' \
                "$DAY_FG" "$cell")
        fi
        i=$((i+3))
    done
    day_rows+="${day_rows:+$'\n'}${row}"
done < <(printf '%s' "$cal_output" | sed -n '3,$p')

# Assemble tooltip with newlines between sections
tooltip="${header_span}"$'\n'"${weekday_span}"
[ -n "$day_rows" ] && tooltip="${tooltip}"$'\n'"${day_rows}"

# Time text for the bar — clean, Inter via waybar's font-family.
time_text=$(date '+%H:%M')
